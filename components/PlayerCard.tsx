  ...
  return (
    <div className="flex flex-col h-full bg-[#0a1428] overflow-hidden relative border-r border-[#c5a059]/10">
      ...
      <div className="relative h-[50%] overflow-hidden bg-gradient-to-b from-[#0a1428] to-[#050b18] shrink-0">
        ...
        {/* Radar Chart - 왼쪽 */}
        <div className="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-40 h-40">
        ...
        <div className="absolute right-8 top-1/2 -translate-y-1/2 z-5">
          {!imgError ? (
            <motion.div
              ...
              className="w-44 h-44 rounded-full overflow-hidden border-2 border-white/10 shadow-2xl"
            >
              <img 
                src={player.photoUrl} 
                alt={player.name} 
            ...
            <div className="w-44 h-44 bg-white/5 rounded-full flex items-center justify-center border border-white/10 italic text-[14px] text-gray-700">
              PROFILE ERROR
            </div>
          )}
        </div>
        ...
        <div className="absolute inset-0 z-20 px-8 flex flex-col justify-end pb-6">
          <motion.div 
          ...
          >
            <span className="text-6xl font-black text-[#c5a059] italic leading-none drop-shadow-[0_8px_20px_rgba(0,0,0,1)]">
              {player.number}
            </span>
            <div className="flex flex-col">
              <span className="text-[11px] font-black text-amber-500 tracking-[0.5em] leading-none mb-1 drop-shadow-md">
                {player.position}
              </span>
              <h2 className="text-4xl font-black tracking-tighter text-white uppercase leading-none drop-shadow-[0_4px_10px_rgba(0,0,0,0.8)]">
                {player.name}
              </h2>
            </div>
          </motion.div>
            ...
            {player.keyTraits.slice(0, 3).map((trait, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-1">
                  <div className="w-1.5 h-1.5 rotate-45 bg-[#c5a059] shadow-[0_0_8px_rgba(197,160,89,0.8)] shrink-0" />
                  <span className="text-[11px] font-black text-white uppercase tracking-tight">
                    {trait.name}
                  </span>
                </div>
                <p className="text-[10px] text-gray-400 leading-snug ml-4 font-medium">
                  {trait.description}
                </p>
              </motion.div>
            ))}
